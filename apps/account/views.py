    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not is_vendor and not is_costumer:
                ...
                messages.success(request, 'Account created successfully', extra_tags="is-success")
...
def personal_data(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.save()
            messages.success(request, 'Account has been changed successfully', extra_tags="is-success")
            return redirect('frontpage')
        else:
            print(form.errors)
...
def change_password(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Your password has been changed successfully', extra_tags="is-success")
            return redirect('frontpage')
            ...
            if check_if_user_exists:
                ...
                if user is not None:
                    if not user.is_costumer:
                        ...
                        costumer = Costumer.objects.create(name=user.username, created_by=user)
                        costumer.save()
                        messages.success(request, "You've become a costumer", extra_tags="is-success")
                    else:
                        messages.error(request, 'You are already a costumer', extra_tags='is-danger')
                    return redirect('frontpage')
            ...
            if check_if_user_exists:
                ...
                if user is not None:
                    if not user.is_vendor:
                        ...
                        vendor = Vendor.objects.create(name=user.username, created_by=user)
                        vendor.save()
                        messages.success(request, "You've become a vendor", extra_tags="is-success")
                    else:
                        messages.error(request, 'You are already a vendor', extra_tags='is-danger')
                    return redirect('frontpage')
    else:
        form = AuthenticationForm()
