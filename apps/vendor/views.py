...
def add_product(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'The product has been added successfully', extra_tags='is-success')
...
def edit_vendor(request):
    ...
    if request.method == 'POST':
        ...
        if name:
            vendor.name = name
            vendor.save()
            messages.success(request, 'Your vendor name has been changed', extra_tags="is-success")
            return redirect('vendor_admin')
        ...
        for order in orders:
            for item in order.items.all():
                if item.vendor == vendor:
                    ...
                    item.save()
        messages.success(request, 'The credit has been recovered successfully', extra_tags='is-success')

    else:
        messages.error(request, "you don't have any credit to recover", extra_tags='is-danger')
    return redirect("vendor_admin")
