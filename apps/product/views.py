...
def product(request, category_slug, product_slug, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'The product has been added to the cart', extra_tags='is-success')
...
def save_unsaved_product(request):
    ...
    if request.method == 'POST':
        ...
        if costumer in product.saved_by.all():
            ...
            messages.error(request, "The product has been removed from favorites successfully", extra_tags='is-danger')
        else:
            product.saved_by.add(costumer)
            send_notification(sender=costumer, recipient=product.vendor, action="add_save", product=product)
            messages.success(request, 'The product has been added  to favorites successfully', extra_tags='is-success')
        product.save()
